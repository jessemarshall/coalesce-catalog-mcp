...
| `COALESCE_CATALOG_READ_ONLY` | When `true`, every mutation tool is filtered out at server registration time (64 tools → 40). | `false` |
...
- **`COALESCE_CATALOG_READ_ONLY=true`** hides all 24 mutation tools at server registration time. Use it for audits, agent sandboxes, or pairing with a prod token. When set, the server registers 40 tools instead of 64.
