  ...
  return {
    ...
    config: {
      ...
      description:
        ...
        "Output also includes a `stats` block with `totalColumnsReached` (column nodes excluding root), `totalDashboardFieldsReached` (dashboard-field endpoints across both directions), and the resolved `maxNodes` ceiling — useful for ranking columns by graph reach without re-running the walk.\n\n" +
